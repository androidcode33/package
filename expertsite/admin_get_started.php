      ...
      <div class="span12">
        <div class="nonboxy-widget">
          <div class="widget-head">
            <h5><i class="black-icons blocks_images"></i>GET STARTED PAGE </h5>
          </div>
          <div class="widget-content">
            <div class="widget-box">
			<?php
					  $conn=connect_to_db();
			$sql23 = mysqli_query($conn,"SELECT * FROM get_started where id=1") or die(mysqli_error()); 
			$row = mysqli_fetch_assoc( $sql23); 
			if(array_key_exists('send', $_POST)){
			$title=mysqli_real_escape_string($_POST['title']);
			$content=mysqli_real_escape_string($_POST['content']);			   	 
			$conde=mysqli_query($conn,"update get_started set title='$title', content='$content' WHERE id='1'"); 
				if(!$conde){
