...
.style15 {font-size: 18px; font-family: "Times New Roman", Times, serif; }
		...
		<script type="text/javascript">
			...
			WYSIWYG.attach('textarea3'); // small setup
      ...
      <div class="span12">
        <div class="nonboxy-widget">
          <div class="widget-head">
            <h5><i class="black-icons blocks_images"></i>HOW IT WORKS PAGE </h5>
          </div>
          <div class="widget-content">
            <div class="widget-box">
		<?php
					  $conn=connect_to_db();
		$sql23 = mysqli_query($conn,"SELECT * FROM how_it_works where id=1") or die(mysqli_error()); 
		$row = mysqli_fetch_assoc( $sql23); 
		if(array_key_exists('send', $_POST)){	
		$title=mysqli_real_escape_string($_POST['title']); 
		$content=mysqli_real_escape_string($_POST['content']);
		$content1=mysqli_real_escape_string($_POST['content1']);
		$content2=mysqli_real_escape_string($_POST['content2']);				   	 
		$conde=mysqli_query($conn,"update how_it_works set title='$title', content='$content', content1='$content1', content2='$content2' WHERE id='1'"); 
		if(!$conde){
		...
		echo "<div class='success' align='center'>Operation completed successfully!.<br><br> <a href='admin'>Click Here</a></div>";			
              ...
              <form action="" method="post" enctype="multipart/form-data" name="theForm" class="form-horizontal" id="theForm" onsubmit="return validateFormOnSubmit(this)">
                ...
                <fieldset>
                  ...
                  <div class="control-group">
                    <label class="control-label">Introduction Content </label>
                    <div class="controls">
                      <textarea id="textarea1" name="content" style="width:90%;height:200px;"><?php echo $row['content'];?></textarea>
                    </div>
                  </div>
				  <div class="control-group">
                    <label class="control-label">Left Content </label>
                    <div class="controls">
                      <textarea id="textarea2" name="content1" style="width:90%;height:200px;"><?php echo $row['content1'];?></textarea>
                    </div>
                  </div>
				  <div class="control-group">
                    <label class="control-label">Right Content </label>
                    <div class="controls">
                      <textarea id="textarea3" name="content2" style="width:90%;height:200px;"><?php echo $row['content2'];?></textarea>
                    </div>
                  </div>
