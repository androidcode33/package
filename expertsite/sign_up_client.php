...
<body>
    ...
    <div class="section secondary-section " id="portfolio">
        ...
        <div class="container">
			<div class="row-fluid">
             <div class="span3">
                ...
                <h5>If you have a degree or experience in any of the following fields, feel free to apply.</h5>
					<?php 	$query1 = "SELECT * FROM discipline order by name DESC limit 20"; $query1 = mysqli_query($conn,$query1) or die(mysqli_error());?>
				<ul class="clip_icn">
			...
			<div class="span6">	
				<div class="title2"> Create Client Account </div>
				<form class="form-horizontal" enctype="multipart/form-data" action="" method="post" name="theForm" id="theForm" onSubmit="return validateForm(this)">
					...
					<?php		$conn=connect_to_db();
								...
								$conn=connect_to_db();	
									...
									if($pass!=$pass2){ echo '<div id="errorSend" class="alert alert-error"><strong>Error! </strong> Your password does not match. </div>';}
									elseif($numRow>0){
										echo '<div id="errorSend" class="alert alert-error"><strong>Error! </strong> The email '.$email.' is used. Please use any other email, each email is entitled to one account!</div>';
										}
									else{	
										$condetails=mysqli_query($conn,"INSERT INTO user (fname,lname,sex,country, city,email,password,phone1,role, status, date, hash_code) values('$fn','$ln','$sex','$country','$city','$email','".md5($pass)."','$phone1', 'Client', 'Approved', '$date','$hash_code')");
...
//
											$headers = 'From:noreply@expertcage.com' . "\r\n"; // Set from headers
					...
					<div class="control-group">
						<label class="control-label req">Password :</label>
                       	<div class="controls">
							...
							value="<?php if(array_key_exists('send', $_POST)){echo $_POST['password1'];}?>"/>
					...
					<div class="control-group">
						<label class="control-label req">Repeat Password :</label>
                       	<div class="controls">
							...
							value="<?php if(array_key_exists('send', $_POST)){echo $_POST['password2'];}?>"/>
