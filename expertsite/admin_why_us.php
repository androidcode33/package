      ...
      <div class="span12">
        <div class="nonboxy-widget">
          <div class="widget-head">
            <h5><i class="black-icons blocks_images"></i>WHY US PAGE </h5>
          </div>
          <div class="widget-content">
            <div class="widget-box">
		<?php
					  $conn=connect_to_db();
		$sql23 = mysqli_query($conn,"SELECT * FROM why_us where id=1") or die(mysqli_error()); 
		$row = mysqli_fetch_assoc( $sql23); 
		
		if(array_key_exists('send', $_POST)){
		$title=mysqli_real_escape_string($_POST['title']); 
		$content=mysqli_real_escape_string($_POST['content']);			   	 
		$conde=mysqli_query($conn,"update why_us set title='$title',content='$content' WHERE id='1'"); 
		if(!$conde){
		echo "Please try again" .mysqli_error(); echo "</p>";}
		else{	echo "<div class='success' align='center'>Operation completed successfully!.<br><br> <a href='admin'>Click Here</a></div>";			
    			}}?>
